# @taktikorg/iusto-pariatur

[![NPM Package Version](https://img.shields.io/npm/v/@taktikorg/iusto-pariatur)](https://www.npmjs.com/package/@taktikorg/iusto-pariatur)
[![Build Status](https://github.com/taktikorg/iusto-pariatur/workflows/CI/badge.svg)](https://github.com/taktikorg/iusto-pariatur/actions?query=workflow%3ACI)
...
```
npm i @taktikorg/iusto-pariatur
```
...
const SonicBoom = require('@taktikorg/iusto-pariatur')
...
* `retryEAGAIN(err, writeBufferLen, remainingBufferLen)`: a function that will be called when @taktikorg/iusto-pariatur
    write/writeSync/flushSync encounters a EAGAIN or EBUSY error. If the return value is
    true @taktikorg/iusto-pariatur will retry the operation, otherwise it will bubble the
    error. `err` is the error that caused this function to be called,
    `writeBufferLen` is the length of the buffer @taktikorg/iusto-pariatur tried to write, and
    `remainingBufferLen` is the length of the remaining buffer @taktikorg/iusto-pariatur didn't try to write.
